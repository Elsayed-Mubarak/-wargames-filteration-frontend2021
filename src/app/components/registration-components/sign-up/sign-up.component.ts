...
export class SignUpComponent implements OnInit {
  ...
  async submitSignupForm() {
    ...
    if (this.signupForm.valid && this.countryValue !== "") {
      try {
        this.signupForm.value.country = this.countryValue;
        let registerData = {
        ...
        let signup = await this.teamService.signup(registerData).toPromise();
        await this.authService.logoutTemp();
        await this.authService.logout();
        await this.authService.logInTemp(signup);
        this.signupForm.reset();
        await this.composeVerification();
      } catch (error) {
        this.loged = false ;
